...
function fantasyMap() {
  ...
  const drag = d3.drag()
    ...
    .subject(() => { const p = [d3.event.x, d3.event.y]; return [p, p]; })
  ...
  function placePoints() {
    ...
    points = [];
    const radius = 5.9 / graphSize; // 5.9 is a radius to get 8k cells
    ...
    if (customization > 0) {
      ...
      if (customization === 1) {
        ...
        if (brushId === 'brushHill' || brushId === 'brushPit') {
          radius = Math.pow(brushPower.value * 400, 0.5);
        }
  ...
  function defineBrushSelection(center, r) {
    ...
    selection = $.grep(selection, e => cells[e].height >= 0.2);
  ...
  function updateCellsInRadius(cell, source) {
    ...
    selection.map((s) => {
      ...
      if (brush === 'brushElevate') {
        if (cells[s].height < 0.2) { cells[s].height = 0.2; } else { cells[s].height += power; }
      }
      if (brush === 'brushDepress') { cells[s].height -= power; }
      if (brush === 'brushAlign') { cells[s].height = sourceHeight; }
      if (brush === 'brushSmooth') {
        const heights = [cells[s].height];
        cells[s].neighbors.forEach((e) => { heights.push(cells[e].height); });
  ...
  function defineHeightmap() {
    ...
    if (lockTemplateInput.getAttribute('data-locked') == 0) {
      ...
      if (rnd > 0.9) { templateInput.value = 'Volcano'; } else if (rnd > 0.7) { templateInput.value = 'High Island'; } else if (rnd > 0.5) { templateInput.value = 'Low Island'; } else if (rnd > 0.35) { templateInput.value = 'Continents'; } else if (rnd > 0.01) { templateInput.value = 'Archipelago'; } else { templateInput.value = 'Atoll'; }
    }
  ...
  function templateContinents(mod) {
    ...
    const count = Math.ceil(Math.random() * 6 + 2);
    addStrait(count);
    smoothHeights(3);
    addPit(rn(18 * mod));
    addRange(rn(-14 * mod));
  ...
  function addMountain() {
    ...
    const height = Math.random() * 0.1 + 0.9;
      ...
      if (start && end) {
        for (let l = 0; start != end && l < 10000; l++) {
          ...
          cells[start].neighbors.forEach((e) => {
            ...
            if (Math.random() > 0.8) { diff /= 2; }
            if (diff < min) { min = diff, start = e; }
          });
          range.push(start);
        }
      }
      var change = height || Math.random() * 0.1 + 0.1;
      range.map((r) => {
        let rnd = Math.random() * 0.4 + 0.8;
        if (mod > 0) { cells[r].height += change * rnd; } else if (cells[r].height >= 0.1) { cells[r].height -= change * rnd; }
  ...
  function addStrait(width) {
    ...
    let start = diagram.find(top, graphHeight * 0.2).index;
    const end = diagram.find(bottom, graphHeight * 0.8).index;
    let range = [];
    ...
    for (; width > 0; width--) {
      range.map((r) => {
        cells[r].neighbors.forEach((e) => {
          ...
          cells[e].used = session;
          query.push(e);
          const height = cells[e].height * 0.23;
  ...
  function addPit(count, height, cell) {
    ...
    for (let c = 0; c < count; c++) {
      ...
      let start = cell;
      if (!start) {
        const lowlands = $.grep(cells, e => (e.height >= 0.2));
  ...
  function smoothHeights(fraction) {
    ...
    cells.map((i) => {
      const heights = [i.height];
      i.neighbors.forEach((e) => { heights.push(cells[e].height); });
  ...
  function disruptHeights() {
    cells.map((i) => {
      ...
      const rnd = rn(2 - Math.random() * 4) / 100;
  ...
  function reduceClosedLakes() {
    ...
    for (let c = 0; c < cells.length && lakesNow > 0; c++) {
      ...
      if (ocean !== null && lake !== null) {
        ...
        cells[c].fn = ocean;
        cells[c].ctype = -1;
        cells[c].neighbors.forEach((e) => { if (cells[e].height >= 0.2) cells[e].ctype = 2; });
      }
    }
  ...
  function updateHeightmap() {
    cells.map((c) => {
      ...
      if (cell.size()) {
        if (height < 0.2) { cell.remove(); } else { cell.attr('fill', clr).attr('stroke', clr); }
      } else if (height >= 0.2) {
  ...
  function updateHistory() {
    ...
    const elevationAverage = rn(d3.mean(heights), 2);
    const landRatio = rn(landCells / cells.length * 100);
    landmassCounter.innerHTML = `${landCells} (${landRatio}%); Average Elevation: ${elevationAverage}`;
    if (landCells > 100) { $('#getMap').attr('disabled', false).removeClass('buttonoff').addClass('glow'); } else { $('#getMap').attr('disabled', true).addClass('buttonoff').removeClass('glow'); }
  ...
  function restoreHistory(step) {
    ...
    undo.disabled = templateUndo.disabled = !(historyStage > 1);
    const heights = history[historyStage - 1];
    if (heights === undefined) { return; }
    cells.map((i, d) => { i.height = heights[d]; });
    updateHeightmap();
  }
  ...
  function drawScaleBar() {
    ...
    const scaleBar = svg.append('g').attr('id', 'scaleBar').on('click', editScale).call(d3.drag().on('start', elementDrag));
  ...
  function elevateLakes() {
    ...
    const lakes = $.grep(cells, e => e.height < 0.2 && !features[e.fn].border);
    lakes.sort((a, b) => b.height - a.height);
    for (let i = 0; i < lakes.length; i++) {
      const heights = [];
      lakes[i].neighbors.forEach((n) => { if (cells[n].height >= 0.2) heights.push(cells[n].height); });
  ...
  function resolveDepressionsPrimary() {
    ...
    for (let l = 0, depression = 1; depression > 0 && l < limit; l++) {
      ...
      for (let i = 0; i < land.length; i++) {
        ...
        const heights = land[i].neighbors.map(n => cells[n].height);
  ...
  function restoreCustomHeights() {
    land.forEach((l) => { if (l.pit) rn(l.height -= l.pit / 50, 2); });
  }
  ...
  function drawRiver(points, width, increment) {
    ...
    for (let p = 1; p < last; p++) {
      ...
      let xPrev = points[p - 1][0],
        yPrev = points[p - 1][1];
      let xNext = points[p + 1][0],
        yNext = points[p + 1][1];
  ...
  function drawRiverSlow(points, width, increment) {
    ...
    const riverPoints = points.map(p => ({ scX: p[0], scY: p[1] }));
  ...
  function addLakes() {
    ...
    while (unmarked.length) {
      ...
      lakeCells.forEach((c) => { cells[c].fn = fn; });
  ...
  function editMonsterIcon() {
    ...
    unselect();
    closeDialogs("#monsterEditor, .stable");
    elSelected = d3.select(this).raise().call(d3.drag().on("start", elementDrag));
    const group = elSelected.node().parentNode.id;
    monsterGroup.value = group;

    // monster editor dialog box preferences
    $("#monsterEditor").dialog({
    ...
    if (modules.editMonsterIcon) { return; }
    modules.editMonsterIcon = true;

    $("#monsterGroups").click(function () {
      $("#monsterEditor > button").not(this).toggle();
      $("#monsterGroupsSelection").toggle();
    });

    // changes icon to different icon based on group
    $("#monsterGroup").change(function () {
      const type = this.value;
      const bbox = elSelected.node().getBBox();
      const cx = bbox.x;
      const cy = bbox.y + bbox.height / 2;
      elSelected.remove();
      elSelected = addMonsterIcon(type, cx, cy);
      elSelected.call(d3.drag().on("start", elementDrag));
    });

    // copies icon and places on map
    $("#monsterCopy").click(function () {
      const group = d3.select(elSelected.node().parentNode);
      ...
      copy.setAttribute("transform", transform);
      group.node().insertBefore(copy, null);
      copy.addEventListener("click", editMonsterIcon);
    });

    $("#monsterAddfromEditor").click(function () {
      clickToAdd(); // to load on click event function
      $("#addMonster").click();
    });

    // removes all icons of this group "monsters" from the map completely
    $("#monsterRemoveGroup").click(function () {
      const group = d3.select(elSelected.node().parentNode);
      const count = group.selectAll("*").size();
      if (count < 2) {
      ...
      const message = "Are you sure you want to remove all '" + monsterGroup.value + "' icons (" + count + ")?";
      alertMessage.innerHTML = message;
      $("#alert").dialog({
    ...
    $("#monsterRemove").click(function () {
      alertMessage.innerHTML = `Are you sure you want to remove the icon?`;
      $("#alert").dialog({
        resizable: false, title: "Remove monster icon",
        buttons: {
          Remove: function () {
            $(this).dialog("close");
            elSelected.remove();
  ...
  function rankPlacesGeography() {
    ...
    land.map((c) => {
      ...
      if (c.ctype && Math.random() < 0.8 && !c.river) {
        ...
        const neighbEv = c.neighbors.map((n) => { if (cells[n].height >= 0.2) return cells[n].height; });
  ...
  function locateTowns() {
    ...
    manors.forEach((m) => { manorTree.add([m.x, m.y]); });
  ...
  function checkAccessibility() {
    ...
    for (let f = 0; f < features.length; f++) {
      ...
      if (manorsOnIsland.length > 0) {
        const ports = $.grep(manorsOnIsland, p => p.port);
        if (ports.length === 0) {
          const portCandidates = $.grep(manorsOnIsland, c => c.harbor && c.ctype === 1);
          if (portCandidates.length > 0) {
            ...
            manorsOnIsland.map((e) => { e.score -= 2; });
          }
        }
      }
    }
  ...
  function generateSmallRoads() {
    ...
    for (let f = 0; f < features.length; f++) {
      var manorsOnIsland = $.grep(land, e => e.manor !== undefined && e.fn === f);
  ...
  function findLandPath(start, end, type) {
    // A* algorithm
    const queue = new PriorityQueue({ comparator(a, b) { return a.p - b.p; } });
    const cameFrom = [];
    const costTotal = [];
    costTotal[start] = 0;
  ...
  function defineRegions() {
    ...
    const manorTree = d3.quadtree();
    manors.forEach((m) => { if (m.region !== 'removed') manorTree.add([m.x, m.y]); });
  ...
  function drawRegions() {
    ...
    for (let i = 0; i < e.length; i++) {
      ...
      if (Number.isInteger(lr)) {
        edges[lr].push(p);
        if (rr === undefined) { coastalEdges[lr].push(p); } else if (rr === 'neutral') { neutralEdges.push(p); }
      }
      if (Number.isInteger(rr)) {
        edges[rr].push(p);
        if (lr === undefined) { coastalEdges[rr].push(p); } else if (lr === 'neutral') { neutralEdges.push(p); } else if (Number.isInteger(lr)) { borderEdges.push(p); }
      }
    }
    edges.map((e, i) => {
      if (e.length) {
  ...
  function drawRegion(edges, region) {
    ...
    while (edges.length > 2) {
      ...
      var edgesFormatted = [];
      edgesOrdered.map((e) => { edgesFormatted.push([+e.scX, +e.scY]); });
  ...
  function toggleHeight() {
    ...
    if (!terrs.selectAll('path').size()) {
      cells.map((i, d) => {
        ...
        if (i.lake) {
          const heights = i.neighbors.map((e) => { if (cells[e].height >= 0.2) return cells[e].height; });
  ...
  function toggleOverlay() {
    if (overlay.selectAll('*').size() === 0) {
      ...
      if (type === 'hex') {
        const hexbin = d3.hexbin().radius(size).size([svgWidth, svgHeight]);
  ...
  function drawMonsters() {
    console.time('drawMonsters');
    const trolls = monsters.select("#hill");
    const dragons = monsters.select("#dragon");
    const mermaids = monsters.select("#mermaid");
    monsters.selectAll("g").selectAll("g").remove();
  ...
  function addMonsterIcon(type, cx, cy) {
    const g = monsters.select("#" + type).append("g");
    if (type === "troll") {
      let d = 'M5409 12070 c-169 -21 -303 -78 -436 -185 -155 -124 -232 -223 -338 -435 -120 -239 -168 -300 -299 -378 -95 -57 -164 -129 -281 -293 -195 -272 -254 -341 -357 -410 -60 -41 -98 -76 -98 -91 0 -5 14 -22 32 -38 17 -16 30 -31 27 -33 -2 -2 -44 -25 -92 -51 -385 -208 -590 -472 -703 -908 -8 -34 -43 -115 -76 -182 -33 -67 -85 -184 -115 -261 -89 -229 -130 -298 -223 -384 -329 -300 -440 -514 -515 -1001 -27 -173 -42 -243 -57 -273 -5 -9 -31 -28 -58 -42 -62 -32 -79 -57 -176 -265 -68 -146 -77 -171 -76 -219 2 -56 -17 -104 -43 -115 -9 -3 -29 -6 -45 -6 -16 0 -67 -14 -113 -30 -47 -17 -98 -30 -114 -30 -65 0 -205 -100 -220 -157 -9 -37 2 -103 26 -160 17 -37 37 -62 74 -90 37 -28 56 -50 68 -83 9 -24 22 -46 29 -48 6 -2 32 -48 57 -101 44 -92 46 -99 44 -182 -1 -47 -6 -131 -10 -187 -9 -100 -11 -107 -91 -275 -45 -95 -88 -179 -96 -187 -33 -38 -46 -93 -39 -164 11 -115 39 -188 89 -234 103 -95 195 -75 303 65 36 47 46 42 38 -20 -15 -118 50 -214 160 -237 72 -15 107 -8 195 37 88 46 105 72 157 246 22 73 43 127 50 127 11 0 43 -86 43 -116 0 -23 54 -77 95 -95 48 -22 128 -24 182 -5 62 22 182 114 203 156 l19 37 88 7 c118 9 161 22 199 60 36 36 54 102 54 196 0 33 5 90 12 128 6 37 8 80 5 95 -8 30 -52 57 -107 66 -44 8 -48 32 -24 122 12 46 18 110 20 226 1 89 5 166 8 171 3 5 27 10 52 10 26 1 62 6 81 13 19 6 37 8 41 4 4 -3 5 -53 3 -111 -2 -78 -8 -117 -23 -150 -14 -32 -21 -77 -25 -153 -6 -115 3 -162 41 -216 18 -26 20 -41 17 -139 -3 -96 -6 -114 -26 -140 -34 -47 -45 -91 -45 -186 0 -112 21 -184 66 -225 l34 -31 0 -97 c0 -87 -2 -97 -21 -107 -11 -6 -29 -26 -39 -43 -17 -28 -20 -51 -20 -167 0 -147 9 -177 65 -216 64 -46 63 -45 70 -134 l7 -85 -35 -36 c-37 -38 -67 -116 -67 -175 0 -39 27 -159 42 -188 6 -12 28 -37 50 -56 37 -33
59642,28.298731 273.51242,30.717131 272.64342,37.848431 C 274.81842,38.109331 276.99142,38.370031 279.16642,38.631031 C 273.25242,42.457431 265.37542,47.855031 259.46042,51.681631 C 252.87642,61.654931 254.14842,61.809231 256.98942,67.069231 C 258.98942,70.547931 260.99042,74.026731 262.99042,77.505431 C 273.13542,80.630731 280.53142,84.934131 282.81942,91.593831 C 271.84542,90.278831 263.85742,92.320831 264.03442,97.072831 C 264.25042,100.57183 272.58142,102.12983 275.97342,104.21683 C 282.27442,110.12283 284.34242,115.32383 284.64542,122.64083 C 276.55742,122.20683 265.99942,110.12783 260.38142,121.33683 C 257.70042,126.81983 260.66442,134.24483 267.68642,136.72983 C 280.56542,140.25883 281.77442,157.86283 281.51342,157.86283 C 281.25342,157.86283 253.22242,139.15383 254.63442,156.62083 C 254.16642,164.33683 257.75742,166.05283 260.64242,168.29883 C 269.04042,171.39983 276.20442,175.02983 275.25242,183.95283 C 263.17142,179.60083 257.44142,183.01383 255.94642,189.95383 C 257.51242,194.47583 259.07642,198.99783 260.64242,203.52083 C 276.99142,200.21483 293.34242,196.91183 309.69242,193.60583 C 316.83742,187.09483 320.05542,176.26483 319.34442,164.64683 C 320.19742,147.34083 313.98142,131.99683 336.04242,112.72683 C 382.03942,74.397031 428.82242,44.314831 481.10442,15.410831 C 470.49942,33.084931 462.86742,66.019931 490.62242,68.115631 C 499.52442,68.247531 597.20442,27.151431 597.20442,27.151431 C 563.24342,51.937031 524.17342,86.541431 569.54942,101.50783 C 527.91742,114.46883 493.38742,156.56683 523.63042,190.47583 C 502.02842,184.32383 473.68642,198.24683 468.58042,213.95683 C 493.15342,213.77383 515.36942,212.41483 538.76342,220.47883 C 538.76342,220.47883 603.97942,233.47283 608.42242,204.82583 C 573.59142,212.31283 547.76942,208.85083 546.59042,177.43083 C 546.89342,150.48683 587.37642,135.32683 621.84942,155.51483 C 625.41442,148.20983 628.86042,140.90383 632.42642,133.59883 C 619.81642,135.15883 608.43142,137.29683 595.38142,134.24683 C 621.91242,109.07383 646.87242,84.294231 669.47542,56.372331 z';
      g.append("path").attr("d", d).attr('transform', "translate(" + cx + ',' + cy + ") scale(.01)").attr('fill', '#000').attr('stroke', 'none');
    }
    if (type === "mermaid") {
      let d = 'M193.173,86.053c-0.681,2.277-1.557,4.518-2.813,6.479   c-1.26,2.645-4.531,3.424-6.635,0.775c0.804,1.164,1.938,2.47,3.727,2.519c1.772,0.005,3.182-1.14,4.263-2.214   c1.305-1.356,2.383-2.848,3.331-4.41c-1.714,4.752-5.782,9.233-11.313,7.226c9.656,6.953,17.051-2.553,22.981-11.244   c-1.6,3.952-3.234,7.807-5.553,11.297c-3.144,5.387-9.787,8.315-16.143,6.112c5.998,2.837,13.704,0.397,17.55-5.097   c4.112-5.119,6.674-11.118,9.652-16.577c0.103-0.198,0.207-0.394,0.313-0.589c-2.006,7.146-1.631,14.68,6.644,15.636   c-0.419-0.068-3.586-2.883-3.953-5.886c-0.399-3
1.088,9.317-2.53   c1.875-0.996,3.654-2.123,5.364-3.314c-3.429,3.227-6.96,5.888-10.386,7.077c-5.222,1.814-11.82,0.304-14.355-8.558   c-0.123,0.271-0.488,1.479-0.623,2.833c-0.105,1.052-0.027,2.162,0.137,3.197c0.398,2.52,1.829,4.223,1.891,4.319   c1.312,1.996,5.131,4.44,9.033,5.497c3.677,0.994,8.034,0.247,10.018-0.343c12.558-3.731,24.435-17.816,26.552-18.322   c-5.767,5.965-2.56,17.595-12.08,18.653c8.377,2.83,11.415-8.928,17.512-12.454c1.086-0.628,2.213-1.195,3.374-1.708   c-3.713,3.672-5.904,8.177-8.711,11.956c-2.874,4.251-7.726,6.904-13.121,7.153c5.313,0.376,10.93-1.873,14.469-6.065   c3.644-3.969,6.462-8.446,10.505-11.268C180.386,86.049,186.442,86.659,193.173,86.053L193.173,86.053z';
      g.append("path").attr("d", d).attr('transform', "translate(" + cx + ',' + cy + ") scale(.01)").attr('fill', '#000').attr('stroke', 'none');
    }
    g.on("click", editMonsterIcon);
    return g;
  }
  ...
  function clickToAdd() {
    ...
    function addRiverOnClick() {
      ...
      while (cell) {
        ...
        var heights = [];
        cell.neighbors.forEach((e) => { heights.push(cells[e].height); });
        const minId = heights.indexOf(d3.min(heights));
        var min = cell.neighbors[minId];
        let tx = cells[min].data[0],
          ty = cells[min].data[1];
        if (cells[min].height < 0.2) {
          const px = (x + tx) / 2;
          const py = (y + ty) / 2;
          ...
          cell = undefined;
        } else if (cells[min].river === undefined) { cells[min].flux += cell.flux; cell = cells[min]; } else {
          ...
          if (dataRiver.length > riverCellsUpper.length) {
            ...
            if (cells[min].confluence) { cells[min].confluence += riverCellsUpper.length; } else { cells[min].confluence = riverCellsUpper.length; }
            cell = cells[min];
            ...
            if (dataRiverMin.length > 1) {
              ...
              riverEl.remove();
              dataRiverMin.map((c) => { cells[c.cell].river = undefined; });
            }
          } else {
            if (cells[min].confluence) { cells[min].confluence += dataRiver.length; } else { cells[min].confluence = dataRiver.length; }
            cells[min].flux += cell.flux;
  ...
  function loadDefaultFonts() {
    if (!$('link[href="fonts.css"]').length) {
      ...
      fontsToAdd.forEach((f) => { if (fonts.indexOf(f) === -1) fonts.push(f); });
      updateFontOptions();
    }
  }
  ...
  $('#labelEditor .editButton, #labelEditor .editButtonS').click(function () {
    ...
    if (this.id == 'editGroupRemove') {
      ...
      const message = `Are you sure you want to remove all labels (${count}) of that group?`;
      alertMessage.innerHTML = message;
      $('#alert').dialog({
        resizable: false,
        title: 'Remove labels',
    ...
    if (this.id == 'editCopy') {
      ...
      let xn = +elSelected.attr('x') - shift;
      let yn = +elSelected.attr('y') - shift;
    ...
    if (this.id == 'editTextRandom') {
      ...
      if (group.attr('id') === 'countries') {
        const state = $.grep(states, e => (e.name === editText.value))[0];
  ...
  function addFonts(url) {
    ...
    return fetch(url)
      ...
      .then((text) => {
        ...
        let fetched = 0;
        for (const r of styleSheet.cssRules) { FontRule(r); }
  ...
  $('#labelEditor .editTrigger').change(function () {
    ...
    const groupOld = group.attr('id');
    let groupNew = editGroupSelect.value;
    ...
    if (groupOld !== groupNew) {
      const removed = elSelected.remove();
  ...
  // module.exports.saveMap = function () {
    ...
    const line = '\r\n';
    let data = version + line + JSON.stringify(points) + line + JSON.stringify(cells) + line;
  ...
  function applyLoadedData(data) {
    ...
    if (!icons.select('#burgIcons').size()) {
      ...
      icons.select('#burgIcons').select('#capitals').attr('size', 1).attr('fill-opacity', 0.7).attr('stroke-opacity', 1);
      icons.select('#burgIcons').select('#towns').attr('size', 0.5).attr('fill-opacity', 0.7).attr('stroke-opacity', 1);
    }
    ...
    cells.map((e) => { newPoints.push(e.data); });
  ...
  function poissonDiscSampler(width, height, radius) {
    let k = 5, // maximum number of points before rejection
  ...
  $('button, a, li, i').on('click', function () {
    ...
    if (id === 'countriesManually') {
      ...
      states.map((s) => {
        ...
        temp.append('path')
          ...
          .attr('d', `M${polygons[index].join('L')}Z`)
          .attr('fill', s.color).attr('stroke', 'red').attr('stroke-width', 0.7);
      });
    ...
    if (id === 'countriesManuallyComplete') {
      ...
      changedStates.map((s) => { recalculateStateData(s); });
      ...
      $('#alert').dialog({
        ...
        buttons: {
          ...
          Remove() {
            $(this).dialog('close');
            $('#countriesBody').empty();
            manors.map((m) => { m.region = 'neutral'; });
            land.map((l) => { l.region = 'neutral'; });
            states.map((s) => {
              const c = +s.capital;
              if (isNaN(c)) return;
    ...
    if (customization === 1) {
      ...
      if (id === 'perspectiveView') {
        ...
        $('#ySlider').slider({
          ...
          max: 5,
          step: 0.1,
          value: +$('#yHandle').attr('data-value'),
          create() { $('#yHandle').text($('#yHandle').attr('data-value')); },
          slide(event, ui) {
        ...
        $('#scaleSlider').slider({
          ...
          max: 2,
          step: 0.1,
          value: +$('#scaleHandle').attr('data-value'),
          create() { $('#scaleHandle').text($('#scaleHandle').attr('data-value')); },
          slide(event, ui) {
        ...
        $('#heightSlider').slider({
          ...
          max: 50,
          step: 1,
          value: +$('#heightHandle').attr('data-value'),
          create() { $('#heightHandle').text($('#heightHandle').attr('data-value')); },
          slide(event, ui) {
  ...
  function openBrushesPanel() {
    ...
    $('#brushesButtons > button').on('click', function () {
      ...
      if ($(this).hasClass('pressed')) {
        ...
        if ($(this).hasClass('feature')) { rSlider.attr('disabled', true).addClass('disabled'); } else { rSlider.attr('disabled', false).removeClass('disabled'); }
      }
    });
  }
  ...
  function fireTemplateElDist() {
    if (this.value === 'interval') {
      const interval = prompt('Populate a height interval (e.g. from 0.17 to 0.2), without space, but with hyphen', '0.17-0.2');
      if (interval) {
  ...
  $('#templateRun').on('click', () => {
    if (customization !== 1) { return; }
    const steps = $('#templateBody > div').length;
    if (steps) { cells.map((i) => { i.height = 0; }); }
  ...
  $('#templateLoad').on('click', () => { templateToLoad.click(); });
  ...
  function convertImage() {
    ...
    $('#imageConverter').dialog({
      ...
      .on('dialogclose', () => { completeConvertion(); });
  }

  // Load image to convert
  $('#convertImageLoad').on('click', () => { imageToLoad.click(); });
  ...
  function showHeight() {
    const el = d3.select(this);
    const height = rn(el.attr('data-color') * 100);
  ...
  function editCountries() {
    ...
    function removeCountry(s) {
      ...
      if (cellsCount) {
        ...
        const burgsSelection = $.grep(manors, e => (e.region === s));
        const urbanFactor = 0.9;
        burgsSelection.map((b) => {
  ...
  function editBurgs(context, s) {
    ...
    burgs.map((b) => {
      ...
      if (b.i === capital) { el.append('<span title="Capital" class="icon-star-empty"></span>'); type = 'c-capital'; } else { el.append('<span class="icon-star-empty placeholder"></span>'); }
  ...
  $('.stable').on('dialogopen', function (event, ui) {
    ...
    $(this).dialog('option', 'position', { my: 'left top', at, of: 'svg' });
  });
  ...
  function editCultures() {
    ...
    const activeCultures = cellsC.reduce((s, v) => { if (v) { return s + 1; } return s; }, 0);
    ...
    function drawCultureCenters() {
      ...
      if (cultureCenters.size()) { cultureCenters.selectAll('*').remove(); } else { cultureCenters = debug.append('g').attr('id', 'cultureCenters'); }
      for (let c = 0; c < cultures.length; c++) {
        cultureCenters.append('circle').attr('id', `cultureCenter${c}`)
          ...
          .on('mousemove', cultureCenterTip).on('mouseleave', () => { tip('', true); })
  ...
  function editNamesbase() {
      ...
      $('#alert').dialog({
        ...
        buttons: {
          Restore() {
            ...
            cultures.forEach((c) => { if (c.base > baseMax) c.base = baseMax; });
            chains = {};
            calculateChains();
            editCultures();
            editNamesbase();
    ...
    $('#namesbaseUpload').on('click', () => { namesbaseToLoad.click(); });
  ...
  function restoreRegions() {
    ...
    states.map((s) => { recalculateStateData(s.i); });
    drawRegions();
  }
  ...
  function applyDefaultStyle() {
    ...
    stateBorders.attr('opacity', 0.8).attr('stroke', '#56566d').attr('stroke-width', 0.7).attr('stroke-dasharray', '1.2 1.5').attr('stroke-linecap', 'butt');
    neutralBorders.attr('opacity', 0.8).attr('stroke', '#56566d').attr('stroke-width', 0.5).attr('stroke-dasharray', '1 1.5').attr('stroke-linecap', 'butt');
    ...
    roads.attr('opacity', 0.9).attr('stroke', '#d06324').attr('stroke-width', 0.35).attr('stroke-dasharray', '1.5').attr('stroke-linecap', 'butt');
    trails.attr('opacity', 0.9).attr('stroke', '#d06324').attr('stroke-width', 0.15).attr('stroke-dasharray', '.8 1.6').attr('stroke-linecap', 'butt');
    searoutes.attr('opacity', 0.8).attr('stroke', '#ffffff').attr('stroke-width', 0.35).attr('stroke-dasharray', '1 2').attr('stroke-linecap', 'round');
    ...
    burgIcons.select('#capitals').attr('size', 1).attr('stroke-width', 0.24).attr('fill', '#ffffff').attr('stroke', '#3e3e4b').attr('fill-opacity', 0.7).attr('stroke-opacity', 1).attr('opacity', 1);
    burgIcons.select('#towns').attr('size', 0.5).attr('stroke-width', 0.12).attr('fill', '#ffffff').attr('stroke', '#3e3e4b').attr('fill-opacity', 0.7).attr('stroke-opacity', 1).attr('opacity', 1);
  ...
  $('#rescaler').change(function () {
    const change = rn((+this.value - 5) / 10, 2);
